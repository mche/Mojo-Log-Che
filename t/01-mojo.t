...
my $log  = Mojo::Log::Che->new(level => 'error', path => $path);
$log->error('Works');
...
#~ like $content,
  #~ qr/\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{5}] \[\d+\] \[e\] Works/,
  #~ 'right error message';
...
my $history;
{
  open my $handle, '>', \$buffer;
  local *STDERR = $handle;
  my $log = Mojo::Log::Che->new->max_history_size(2)->level('info');
  $log->error('First');
  $log->fatal('Second');
  $log->debug('Third');
